-- 031_add_cnpj_cliente.sql
-- CNPJ Cliente (planilha) — match de relatórios e Progressão.

ALTER TABLE prb_deliveries
    ADD COLUMN IF NOT EXISTS cnpj_cliente VARCHAR(14);
...
CREATE INDEX IF NOT EXISTS ix_prb_deliveries_cnpj_cliente
    ON prb_deliveries (cnpj_cliente)
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        INSERT INTO prb_deliveries_audit (
            ...
            dataset_source, dataset_batch_id, dataset_sync_id, cnpj_cliente, created_on_audit, action
        ) VALUES (
            ...
            NEW.dataset_source, NEW.dataset_batch_id, NEW.dataset_sync_id, NEW.cnpj_cliente, NOW(), 'INSERT'
        );
        RETURN NEW;
    ELSIF TG_OP = 'UPDATE' THEN
        INSERT INTO prb_deliveries_audit (
            ...
            NEW.cidade_remetente, NEW.uf_remetente, NEW.peso_taxado, NEW.peso_informado, NEW.raw_json, NEW.synced_at, NEW.source,
            NEW.created_by, NEW.created_on, NEW.modified_by, NEW.modified_on, NEW.enabled, NEW.dt_recebimento, NEW.cliente_conta,
            NEW.dataset_source, NEW.dataset_batch_id, NEW.dataset_sync_id, NEW.cnpj_cliente, NOW(), 'UPDATE'
        );
        RETURN NEW;
    ELSIF TG_OP = 'DELETE' THEN
        INSERT INTO prb_deliveries_audit (
            ...
            OLD.dataset_source, OLD.dataset_batch_id, OLD.dataset_sync_id, OLD.cnpj_cliente, NOW(), 'DELETE'
        );
        RETURN OLD;
    END IF;
    RETURN NULL;
