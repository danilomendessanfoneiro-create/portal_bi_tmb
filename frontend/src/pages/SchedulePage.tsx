...
function AutomationCard({
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setError("");
    try {
      const updated = await updateSchedule(item.job_id, {
        local_time: time,
        enabled,
        frequency: "daily",
      });
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--muted)", fontSize: "0.85rem" }}>
        Cada automação segue o próprio horário, de forma independente. Ambos os
        relatórios são enviados diariamente no horário configurado.
      </p>
    </div>
  );
}
