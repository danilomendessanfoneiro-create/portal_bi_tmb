Sub F_DEMAIS_CLIENTE_1_teste()
  ...
  ' === PADRONIZAR CABEÇALHOS EM MAIÚSCULO ===
    ...
    For Each cab In ws.Rows(1).Cells
        ...
        End Select
    Next cab
    ' === Excluir linhas com STATUS TMS = ENTREGUE ===
Dim colStatus As Range
Dim ultimaLinhaStatus As Long

Set colStatus = ws.Rows(1).Find("STATUS TMS", LookAt:=xlWhole)

If Not colStatus Is Nothing Then
    ultimaLinhaStatus = ws.Cells(ws.Rows.Count, colStatus.Column).End(xlUp).Row

    With ws
        .Rows(1).AutoFilter Field:=colStatus.Column, Criteria1:="ENTREGUE"
        On Error Resume Next
        .Range("A2:A" & ultimaLinhaStatus).SpecialCells(xlCellTypeVisible).EntireRow.Delete
        On Error GoTo 0
